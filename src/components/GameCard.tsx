  ...
  return (
    <div className="group relative flex flex-col rounded-2xl border bg-card text-card-foreground shadow-premium transition-all duration-300 ease-out-back hover:-translate-y-1.5 hover:shadow-premium-hover dark:hover:shadow-premium-dark-hover hover:border-primary/30 overflow-hidden">
      ...
      <div className="relative aspect-[4/3] w-full overflow-hidden bg-muted">
        ...
        {/* Favorite Toggle Button (Bottom-Right of Cover) */}
        <button
          onClick={() => toggleFavorite(game.bggId)}
          aria-label="Toggle Favorite"
          className="absolute right-3 bottom-3 z-10 flex h-7.5 w-7.5 items-center justify-center rounded-full bg-background/90 text-foreground border shadow-sm backdrop-blur-sm transition-all duration-200 hover:scale-110 active:scale-95 cursor-pointer"
        >
          <Heart
            size={14}
        ...
        {/* Interesting (Siguiendo) Toggle Button (Top-Right of Cover) */}
        <button
          onClick={handleToggleInteresting}
          aria-label="Toggle Following Status"
          className={`absolute right-3 top-3 z-10 flex h-7.5 w-7.5 items-center justify-center rounded-full border shadow-sm backdrop-blur-sm transition-all duration-200 hover:scale-110 active:scale-95 cursor-pointer ${
            interesting
